...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    empty(trim($STYLE))) {
        header('Location: create_product.php?error=missing&ORDER_NUM=' . $ORDER_NUM);
        exit();
    }elseif($_FILES["create-img"]["error"] == UPLOAD_ERR_NO_FILE){
        header('Location: create_product.php?error=noimg&ORDER_NUM=' . $ORDER_NUM);
        exit();
    }
    ...
    do{
        ...
        VALUES ('$FURNITURE_NAME', '$PRICE', '$MATERIAL', '$COLOR', '$STYLE', '$FEATURES');";
        ...
        header("Location: create_product.php?ORDER_NUM=" . $ORDER_NUM);
    }while(false);
}
